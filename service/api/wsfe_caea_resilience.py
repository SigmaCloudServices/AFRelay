...
async def queue_solicitar_caea(
    ...
    logger.info("Queued CAEA solicit for tenant %s cycle id=%s", tenant["id"], cycle["id"])
    return {"status": "queued", "cycle": cycle, "job": job}


@router.post("/wsfe/caea/queue/issue-local")
async def queue_issue_local_invoice(
    payload: QueueIssueLocalInvoiceRequest,
    billing=make_billing_dependency("wsfe_caea_queue"),
) -> dict:
    init_db()
    tenant = billing["tenant"]
    data = payload.model_dump()
    ...
    debit_after_call(billing, reference=f"caea_queue:{data['Cuit']}:{next_nro}")
    return {
        "status": "queued",
